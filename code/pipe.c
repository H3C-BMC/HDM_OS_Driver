...
STATIC INT SendPipeDataToSlave(UCHAR *pucBuff, UINT uiLen, UINT uiTimeout)
{
    ...
    if (0 == lTime)
    {
        iRet = down_timeout(&g_stSendDataSem, MAX_SCHEDULE_TIMEOUT); /* 无限等待(死等) */
		if (-ETIME == iRet)
		{
			/* 超时处理 */
			if (pucBuff)
	...
	{
        start_jiffies = jiffies; /* 记录开始时间(jiffies单位) */
	    iRet = down_timeout(&g_stSendDataSem, lTime); /* 超时等待 */
        if (-ETIME == iRet)
		{
			/* 超时处理 */
            if (pucBuff)
	    ...
	    /* lTime - actual_wait_jiffies理论上恒大于0,当down_timeout(&g_stSendDataSem, lTime)执行成功说明未超时,当执行失败则说明超时已返回 */
		if (0 >= lTime)
		{
			/* 剩余lTime的负值校验 */
			if (pucBuff)
